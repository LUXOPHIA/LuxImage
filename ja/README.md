...
- **色管理**（`LUX.Color.Space`）。画像は色空間（sRGB ・ Display P3 ・ Adobe RGB ・ Rec.2020 ・ ProPhoto RGB ・ ACEScg とその線形版、あるいは自作）を持て、保存時に PNG（`sRGB` / `iCCP`）と JPEG（APP2 ICC）へ埋め込まれ、読み込み時に復元される。ビューアは GPU で ── 画像の伝達関数 → 原色の行列 → 表示の伝達関数 ── 窓の乗っているモニターに Windows が割り当てた ICC プロファイル、または指定した空間へ変換する。画素値は一切変えない。
...
と選ぶ。ここで $N$ は段数である。選ばれた段の内部では実効倍率が $s \cdot 2^{l} \in (\tfrac{1}{2}, 1]$ となり、段を拡大することはない。残る高々 2 倍の縮小は GPU に任せる ── 各タイルをミップマップ付きテクスチャとしてトリリニアで描くので、その段と 1 つ粗い段が連続にブレンドされる [2] ── ため、どの倍率でも拡大のボケも縮小のエイリアシングも出ない。$s \geq 1$ では最近傍サンプリング（等倍を超えると画素が四角に見える）となる。
...
・LuxImage/
  ...
  ┃     ┣・LUX.pas                      ･･･ 基本宣言
  ┃     ┣・Color/                       ･･･ 画素型（TByteRGBA など）、色空間（LUX.Color.Space）
...
| `色空間` | 画像の色空間 ── `なし` か、sRGB 〜 ACEScg の 10 プリセット。読み込んだファイルに埋め込まれたプロファイルは対応する項目を選ぶ（無ければ項目を足す）。選択は保存時に埋め込まれ、描画にも使われる。色空間があればビューアは GPU でモニターのプロファイルへ変換し、情報欄に *画像 → 表示* を示す |
